...
class Usuario_model extends CI_Model
{
    ...
    public function updateUsuario($id, $username, $idDepar, $foto_perfil)
    {
        ...
        if ($valido) {
            ...
            ($foto_perfil == 1) ? $usuario->foto_perfil = 'profilepic.png' : $usuario->foto_perfil = $foto_perfil;
            R::store($user);
        } else {
            $e = ($id == null ? new Exception("Nulo") : new Exception("Error al actualizar el perfil de usuario."));
            throw $e;
        }
    }
    ...
    public function getIDByUsername()
    {
        ...
        $usuario = R::findOne('usuario', 'username=?', [
            $username
        ]);
        $userId = $usuario->id;
        return $userId;
    ...
    public function registrarUsuario($nombre, $apellidos, $username, $pwd, $idDepar, $foto_perfil)
    {
        ...
        $userOk = R::findOne('usuario', 'username=?', [
            $username
        ]);
        if ($valido && $userOk == null) {
            ...
            $e = ($nombre == null || $apellidos == null || $username == null || $pwd == null || $idDepar == null ? new Exception("Debes rellenar todos los campos") : new Exception("Error al actualizar el perfil de usuario."));
            throw $e;
        }
    }
